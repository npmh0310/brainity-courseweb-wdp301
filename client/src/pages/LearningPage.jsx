    ...
    useEffect(() => {
        const fetchCourse = async (courseId) => {
            ...
            try {
                ...
                if (response_Course.data.success && response_LessonProgress.data.success) {

                    const sectionsWithProgress = response_Course.data.data.sections.map(section => ({
                        ...section,
                    ...
                    <div className='relative w-full flex'>
                        <div className={`${showCourse ? 'w-9/12' : 'w-full'}  flex flex-col`}>
                            ...
                            <div className=' relative bg-black video w-full h-[600px]'>
                                <Routes>
                                    {course.sections && course.sections.length > 0 && course.sections.map((section, sectionIndex) => (
                                        section.lessons && section.lessons.length > 0 && section.lessons.map((lesson, lessonIndex) => (
                                            <Route
                                                key={`${sectionIndex}-${lessonIndex}`}
                                                path={`lesson/${lesson._id}`}
                                                element={<VideoChaper courseProgress={courseProgress} courseId={courseId} lesson={lesson} />}
                                            />
                                        ))
                                    ))}
                            ...
                            <div className={`${!showCourse ? 'lg:w-9/12' : 'sm:w-full'} px-4 mx-auto mb-6  flex flex-col`}>
                                <div className=' px-4 min-w-[880px]  flex border-b  justify-start items-center gap-4 '>
                                    <NavLink to={`#search`} className={section === 'search' ? 'py-4 px-1 text-center font-semibold text-lg border-b-2 border-black cursor-pointer' : "py-4 px-1 text-center font-semibold text-lg cursor-pointer"}><Search size={16} /></NavLink>
                                    {!showCourse && <NavLink to={`#courseContent`} className={section === 'courseContent' ? 'py-4 px-1 text-center font-semibold text-lg border-b-2 border-black cursor-pointer' : "py-4 px-1 text-center font-semibold text-lg cursor-pointer"}>Course Content</NavLink>}
                                    <NavLink to={`#overview`} className={section === 'overview' ? 'py-4 px-1 text-center font-semibold text-lg border-b-2 border-black cursor-pointer' : "py-4 px-1 text-center font-semibold text-lg cursor-pointer"}>Overview</NavLink>
                                    <NavLink to={`#exercise`} className={section === 'exercise' ? 'py-4 px-1 text-center font-semibold text-lg border-b-2 border-black cursor-pointer' : "py-4 px-1 text-center font-semibold text-lg cursor-pointer"}>Exercise</NavLink>
                        ...
                        {showCourse && 
                            <div className={`px-2 flex  flex-col overflow-y-auto fixed right-0 ${isScrolled ? 'top-0' : ''} h-screen w-3/12 z-10 bg-white border-x animate-transCourse `}>
