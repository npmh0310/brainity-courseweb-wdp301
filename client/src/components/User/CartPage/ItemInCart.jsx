...
import { moveCourseToCart, removeCourse, saveCourseForLater } from '../../../redux/features/cartSlice'
  ...
  return (
    ...
    <div className=' py-4 border-t flex justify-between items-start'>
      <div className=' basis-1/6 w-full pr-2'>
        <img className=' w-full h-full object-cover' src={course.imageUrl} alt="" />
      </div>
      <div className=' basis-4/6 w-full flex flex-col lg:flex-row gap-y-3'>
        <div className=' basis-5/6 banner flex flex-col px-2 mb-2 '>
          <Link to={`/course/${course._id}`} className=' text-[16px] text-black font-bold mb-1'>
            {course.courseName}
          </Link>
        ...
        <div className=' flex text-sm px-2 lg:flex-col gap-x-3 gap-y-1 '>
          <span className=' text-third lg:text-end hover:text-primary cursor-pointer transition-colors ease-in-out' onClick={remove}>
            Remove
          </span>
          { later === true ?
            <span className='  text-third lg:text-end hover:text-primary cursor-pointer transition-colors ease-in-out' onClick={saveForLater}>
              Save for later
            </span> :
            <span className='  text-third lg:text-end hover:text-primary cursor-pointer transition-colors ease-in-out' onClick={moveToCart}>
              Move to Cart
            </span>
          }
